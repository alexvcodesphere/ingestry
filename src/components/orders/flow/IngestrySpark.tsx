    ...
    return (
        ...
        >
            <div className={`shrink-0 h-6 w-6 rounded-full flex items-center justify-center ${
                ...
                isTool ? "bg-amber-100 dark:bg-amber-900/30 text-amber-600 dark:text-amber-400" :
                isQuestionPrompt ? "bg-blue-100 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400" :
                isSystem ? "bg-muted text-muted-foreground" :
                "bg-primary/10 text-primary"
            }`}>
                {isUser ? <User className="h-3 w-3" /> :
                 isTool ? <Wrench className="h-3 w-3" /> :
                 isQuestionPrompt ? <HelpCircle className="h-3 w-3" /> :
                 isSystem ? <Info className="h-3 w-3" /> :
            ...
            <div className={`max-w-[85%] rounded-lg px-3 py-2 text-sm ${
                ...
                isSystem ? "bg-muted/50 text-muted-foreground italic text-xs" :
                "bg-muted"
            }`}>
                {isThinking ? (
                            ...
                            <div className={`flex-1 p-3 space-y-3 ${messages.length > 0 ? 'overflow-y-auto' : 'overflow-hidden'}`}>
                                {messages.length === 0 ? (
                                    <div className="flex flex-col items-center justify-center h-full py-12 text-center">
                                        ...
                                        <h4 className="font-medium text-sm mb-1">Ask Spark</h4>
                                        <p className="text-sm text-muted-foreground max-w-[200px]">
                                            Describe what you want to change or ask a question about your data
                                        </p>
